...
3. In the Supabase SQL Editor, run these files **in order**:
   ...
   - `extra_schema_46.sql` — Campus canteen ordering: students/teachers order from an admin-managed menu (seeded from the school's Afrikme menu), delivered on campus, with an optional payment-receipt upload. Admin gets an Orders tab to track status (pending → preparing → ready → delivered) and manage the menu. Receipts are private, resolved to a short-lived signed URL — same pattern as assignment files.
