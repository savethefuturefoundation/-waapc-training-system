...
3. In the Supabase SQL Editor, run these files **in order**:
   ...
   - `extra_schema_44.sql` — Freezes the GED weekday subject rotation. The timetable used to relabel itself every week (a client-side formula recomputed which subject occupied each class slot, keyed off the calendar week) — students and teachers found this confusing, since Monday's schedule wouldn't stay Monday's schedule. Bakes in exactly what was showing the week this ran as the new fixed schedule; the app no longer recomputes it at all.
   - `extra_schema_45.sql` — Class Notes: a lesson-plan/materials feed per class (subject), separate from Assignments — no due date, no grading, no submission, just a running feed of notes with an optional link and/or document attachment. Read by everyone (like the timetable); posting is scoped to admin and, for a teacher, only the subject(s) they're assigned to (reuses `teacher_scoped_to_subject()` from `extra_schema_32.sql`).
   - `extra_schema_46.sql` — Campus canteen ordering: students/teachers order from an admin-managed menu (seeded from the school's Afrikme menu), delivered on campus, with an optional payment-receipt upload. Placing an order saves it here (so admin can track status: pending → preparing → ready → delivered) *and* opens WhatsApp with the order pre-filled to the canteen's own number — the same behavior as the standalone Afrikme order page, just embedded — so the order still actually reaches the canteen. Receipts are private, resolved to a short-lived signed URL — same pattern as assignment files.
4. In Supabase **Authentication → Providers → Email**, turn off "Confirm email" (so a student's, teacher's, or parent's first-login signup works immediately).
   - Also in **Authentication → URL Configuration**, add your site's URL (e.g. `https://your-project.pages.dev`, and `http://localhost:5173` for local dev) to **Redirect URLs** — required for "Forgot password?" to work, since Supabase only redirects a password-reset link back to an allowlisted URL.
